...
UPDATE_RELEVANT_DIRS = ("pi/backend", "pi/frontend", "pi/oled", "pi/leds", "pi/screen", "pi/scripts", "pi/systemd")
...
def update_apply():
    ...
    update_script = str(REPO_PATH / "pi" / "scripts" / "update.sh")
    if not Path(update_script).exists():
        raise HTTPException(status_code=500, detail="update.sh nicht gefunden")
