...
static String renderConfigPage(const RuntimeConfig& c, const String& errorMsg = "") {
  ...
  h += F("<fieldset><legend>Ring 1 Standby-Lichteffekt (Twinkle)</legend>");
  ...
  h += F("<label>Anzahl aktiver LEDs im Twinkle: Minimum</label><input type='number' min='0' max='25' name='standbyOnMin' value='"); h += String(c.standbyOnMin); h += F("'><small>Min: 0, Max: 25. Untere Grenze fuer gleichzeitig leuchtende LEDs im Standby. Wirkt nach Speichern sofort.</small>");
  h += F("<label>Anzahl aktiver LEDs im Twinkle: Maximum</label><input type='number' min='0' max='25' name='standbyOnMax' value='"); h += String(c.standbyOnMax); h += F("'><small>Min: 0, Max: 25. Obere Grenze fuer gleichzeitig leuchtende LEDs (muss >= Minimum sein). Wirkt nach Speichern sofort.</small>");
