...
static inline void pixelsClear() {
  primaryLedRing.strip->clear();
}

static inline void pixelsShow() {
  primaryLedRing.strip->show();
}
...
static void ledsSetMode(LedMode m) {
  ...
  ledTickMs = now;
  ledFlip = false;
  if (m == LedMode::READY_GREEN_BLINK) ledFlip = true; // startet sichtbar "an"
  ledSpinIdx = 0;
  ledFrameDirty = true;
...
static void ledService(uint32_t now) {
  ...
  switch (ledMode) {
    ...
    case LedMode::ERROR_BLINK_RED:
      ...
      if (ledFrameDirty) {
        pixelsClear();
        if (ledFlip) pixelsFill(colorRed);
      }
      break;
    ...
    case LedMode::OK_ALT_GB:
      ...
      if (ledFrameDirty) {
        pixelsClear();
        if (ledFlip) pixelsSet(ALT_PATTERN_A, sizeof(ALT_PATTERN_A), colorGreen);
        else pixelsSet(ALT_PATTERN_B, sizeof(ALT_PATTERN_B), colorBlue);
      }
      break;
    ...
    case LedMode::READY_GREEN_BLINK:
      ...
      if (ledFrameDirty) {
        pixelsClear();
        if (ledFlip) pixelsFill(colorGreen);
      }
      break;

    case LedMode::GLASS_GREEN_SOLID:
      if (ledFrameDirty) {
        pixelsFill(colorGreen);
      }
      break;

    case LedMode::TIMING_BLUE_SPINNER:
      if (now - ledTickMs >= 180) {
        ledTickMs = now;
        ledSpinIdx = (uint8_t)((ledSpinIdx + 1) % PIXEL_COUNT);
        ledFrameDirty = true;
      }
      if (ledFrameDirty) {
        pixelsClear();
        primaryLedRing.strip->setPixelColor(ledSpinIdx, colorBlue);
      }
      break;

    case LedMode::RESULT_FLASH_GB_ONCE:
      if (now - ledTickMs < 200) {
        if (ledFrameDirty) {
          pixelsFill(colorCyan);
        }
      } else {
